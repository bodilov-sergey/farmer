const priceSwitch = () => {
    [].forEach.call(document.querySelectorAll("form.card"), function (form) {
        form.addEventListener("change", function () {
            try {
                let sum = form.querySelector("input[data-price]:checked").getAttribute("data-price");
                const priceNew = form.querySelector(".price-new");
                const priceOld = form.querySelector(".price-old");
                let numberFrom = Number(typeof priceNew.dataset.from === "undefined" ? 0 : priceNew.dataset.from);
                ...
                const tick = function (now) {
                    ...
                    priceOld.textContent = (value * 1.428571428571429).toFixed();
                    if (progress < 1) {
                        requestAnimationFrame(tick);
                    } else {
                        priceNew.dataset.from = numberTo.toFixed();
                        priceOld.textContent = Number(numberTo * 1.428571428571429).toFixed();
                    }
                };
                requestAnimationFrame(tick);
            } catch (e) {
